...
// 👈 حط الـ Client ID بتاع بايبال هنا (هقولك تجيبه منين تحت)
const PAYPAL_CLIENT_ID = "test"; // غير الكلمة دي للـ ID بتاعك
...
export default function Home() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.detail) {
        ...
        alert("مشكلة في الاتصال بالسيرفر 🚨 تأكد إن البايثون شغال.");
      }
    } finally {
      setLoading(false);
    }
  ...
  // الدالة دي هتتنده أوتوماتيك لما بايبال يسحب الفلوس بنجاح
  const handleSuccessfulPayment = async () => {
    if (!user) return;
    try {
      const formData = new FormData();
      formData.append("email", user.email);
      await axios.post('http://localhost:8080/upgrade/', formData);
      setUser({ ...user, plan: 'PRO' });
      alert("🎉 مبروك يا هندسة! تم الدفع بنجاح وحسابك بقى PRO.");
      setView('grid');
    } catch (err) {
      alert("تم الدفع بس حصلت مشكلة في تفعيل الحساب، كلم الدعم الفني!");
    }
  };
  ...
  const handleProcess = async () => {
    ...
    if (!user) {
      alert("لازم تسجل دخول الأول عشان تستخدم الأدوات 🔒");
      setView('login');
      return;
    }
    ...
    try {
      const res = await axios.post(`http://localhost:8080/${activeTool.id}/`, formData, { 
      ...
      if (err.response && err.response.data && err.response.data.detail) {
        ...
        alert("الملف ده مش مدعوم أو تالف ❌");
      }
    } finally {
      setLoading(false);
    }
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-16 relative z-50">
            ...
            {!user ? (
              <>
                <form onSubmit={handleAuth} className="flex flex-col gap-4 justify-center max-w-md mx-auto mb-6">
                  <input type="email" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} placeholder="الإيميل" required className="w-full bg-gray-900/80 border-2 border-gray-700 rounded-2xl p-5 text-xl text-white outline-none focus:border-cyan-500 transition-colors text-center font-bold" />
                  <input type="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} placeholder="كلمة السر" required className="w-full bg-gray-900/80 border-2 border-gray-700 rounded-2xl p-5 text-xl text-white outline-none focus:border-cyan-500 transition-colors text-center font-bold" />
              ...
              <div className="mb-16 text-cyan-400 font-bold text-xl flex items-center justify-center gap-2">
                <span>أنت مسجل دخول بإيميل: {user.email}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-right mb-12">
              <div className="p-8 rounded-[2.5rem] bg-gray-900/40 border-2 border-gray-800 backdrop-blur-md">
                ...
                <ul className="space-y-4 mb-8 text-gray-400 font-bold">
                  ...
                  <li>✅ عمليات غير محدودة يومياً ∞</li>
                  <li>❌ مقفول: التشفير واستخراج الصوت</li>
              ...
              <div className="p-8 rounded-[2.5rem] bg-gray-900/60 border-2 border-cyan-500 shadow-[0_0_40px_rgba(6,182,212,0.2)] backdrop-blur-md relative overflow-hidden">
                <div className="absolute top-6 left-[-40px] bg-cyan-500 text-black font-black py-1 px-12 rotate-[-45deg] text-sm shadow-lg">موصى به</div>
                <h3 className="text-3xl font-black mb-4 text-cyan-400">Sabae PRO ⚡</h3>
                <div className="text-5xl font-black mb-6 text-white">5$ <span className="text-lg text-gray-500">/شهر</span></div>
                <ul className="space-y-4 mb-8 text-gray-300 font-bold">
                  <li>✅ <span className="text-cyan-400">كل مميزات المجاني (لا نهائي)</span></li>
                  <li>✅ حماية وفك تشفير الملفات 🔒</li>
                  <li>✅ فصل واستخراج الصوت (MP3) 🎧</li>
                  <li>✅ إزالة الخلفية واستخراج جداول Excel (AI) ✨</li>
                </ul>
                
                {/* 👈 أزرار الدفع الحقيقية بتاعت بايبال هتظهر هنا */}
                {(!user || user.plan === 'Free') && (
                  <div className="mt-6 z-50 relative">
                    {!user ? (
                       <button onClick={() => alert("لازم تسجل دخول الأول عشان تقدر تدفع!")} className="w-full py-4 rounded-xl bg-gray-700 text-gray-400 font-black text-xl cursor-not-allowed border border-gray-600">
                         سجل دخول للاشتراك
                       </button>
                    ) : (
                        ...
                        <PayPalButtons 
                          ...
                          onApprove={async (data, actions) => {
                            if (actions.order) {
                               ...
                               handleSuccessfulPayment(); // الفلوس اتسحبت، نكلم السيرفر يرقيه
                            }
                          }}
                        />
        ...
        {view === 'tool' && (
          <div className="max-w-4xl mx-auto bg-gray-900/60 border-2 border-gray-800/80 rounded-[3rem] p-12 shadow-[0_0_80px_rgba(0,0,0,0.6)] backdrop-blur-xl text-center relative overflow-hidden">
            ...
            <p className="text-gray-400 font-bold mb-10">{user?.plan === 'Free' && !activeTool.isPro ? `متاح لك عدد لا نهائي من العمليات ∞` : user?.plan === 'PRO' ? 'عمليات غير محدودة 🚀' : 'سجل دخولك أو رقي حسابك عشان تستخدم الأداة 🔒'}</p>
            ...
            <div className="relative border-2 border-dashed border-gray-600/60 rounded-[2.5rem] p-10 mb-10 bg-black/30 min-h-[250px] flex flex-col items-center justify-center hover:border-cyan-500 hover:bg-black/50 transition-all duration-300 group">
              {files.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-5 justify-center w-full relative z-50 p-4">
                  ...
                  <button onClick={() => setFiles([])} className="text-red-500 font-bold text-lg w-full mt-6 hover:text-red-400 transition-colors cursor-pointer tracking-wider">× مسح الكل</button>
                </div>
              )}
            </div>
           ...
           {((activeTool as any).inputPlaceholderAr || (activeTool as any).inputPlaceholderEn) && (
            ...
            <div className="flex gap-6 relative z-50">
              <button onClick={() => { setFiles([]); setExtraParam(""); setView('grid'); }} className="flex-1 py-5 rounded-2xl bg-gray-800 font-black text-xl hover:bg-gray-700 transition-all cursor-pointer shadow-lg border border-gray-700">عودة</button>
